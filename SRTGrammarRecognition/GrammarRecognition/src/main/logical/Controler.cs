...
namespace GrammarRecognition.src.main.logical
{
    class Controler
    {
            ...
            String paragraphPath,String outPath, String associateFilePath,bool useAssociate)
        {
            ...
            handleFiles(paragraphPath);
        }
        public void initPragraphs(string filePath)
        {
            paragraphs = new List<Paragraph>();
            sentences = new List<Sentence>();

            PrepareSentences p = new PrepareSentences();
            p.prepareWithFile(filePath,paragraphs, sentences);

        }
        public void handleFiles(string paragraphPath)
        {
            DirectoryInfo dirInfo = new DirectoryInfo(paragraphPath);
            FileInfo[] files = dirInfo.GetFiles();
            for (int i = 0; i < files.Length; i++)
            {
                initPragraphs(files[i].FullName);
                processFile(files[i].Name);
            }
            DirectoryInfo[] dirs = dirInfo.GetDirectories();
            for (int i = 0; i < dirs.Length; i++)
            {
                handleFiles(dirs[i].FullName);
            }
        }
        public void processFile(string fileName)
        {
            ...
            fileName = fileName.Substring(0, pos) + "-加语法" + fileName.Substring(pos, fileName.Length - pos);

            printSentenceWithGrammar(this.outPath + "\\" + fileName);
